 ...
 * @license    MIT license
 */
class Agileo_Web_Controller_Plugin_Acl extends Zend_Controller_Plugin_Abstract
{
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {

        $cache = Zend_Registry::get('cache');
        $cacheIdentity = md5(APPLICATION_NAME.'Acl');
        ...
        if(!$acl->has($resource)) {
            ...
            Agileo_Controller_Plugin_Layout::$prepareLayoutBoxes = false;
            ...
            if (!$acl->isAllowed($role, $resource, $action)) {
                ...
                if ($role == 'guest') {
                    header('Location: /profile/auth/login');
                    exit;
                } else {
                    $request->setModuleName('default');
                    $request->setControllerName('error');
